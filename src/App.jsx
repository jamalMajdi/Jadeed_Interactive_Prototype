...
import { ChevronRight, ChevronLeft, Monitor, Search, Store, User } from 'lucide-react'
...
const ADMIN_ACTIVE = {
  c03: 'c03', c03e: 'c03', c17: 'c03', c04: 'c03',
  c05: 'c05', c05e: 'c05', c06: 'c05',
  c07: 'c07', c07e: 'c07', c08: 'c07',
  c09: 'c09',
  c10: 'c10', c11: 'c10',
  c12: 'c12', c12e: 'c12', c13: 'c12',
  c18: 'c18', c19: 'c19', c14: 'c14', c15: 'c15',
}
...
const APPS = [
  { id: 'cust', name: 'تطبيق العميل', short: 'العميل', hint: 'A-01…A-18', Icon: User, accent: 'bg-jadeed-orange', def: 'a06', groups: GROUPS, screens: SCREENS },
  { id: 'mer', name: 'تطبيق التاجر', short: 'التاجر', hint: 'B-01…B-19', Icon: Store, accent: 'bg-jadeed-purple-light', def: 'b07', groups: MERCHANT_GROUPS, screens: MERCHANT_SCREENS },
  { id: 'adm', name: 'لوحة الإدارة', short: 'الإدارة', hint: 'C-01…C-19', Icon: Monitor, accent: 'bg-jadeed-black', def: 'c03', groups: ADMIN_GROUPS, screens: ADMIN_SCREENS, desktop: true },
]
...
function DeviceStage({ children }) {
  ...
  return (
    <div ref={boxRef} className="flex min-h-0 grow items-center justify-center overflow-hidden p-3 sm:p-5">
      <div className="relative" style={{ width: FRAME_W * scale, height: FRAME_H * scale }}>
        <div className="absolute left-0 top-0" style={{ width: FRAME_W, height: FRAME_H, transform: `scale(${scale})`, transformOrigin: 'top left' }}>
          {children}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="mb-2.5">
      <div className="flex items-center justify-between px-2 pb-1 pt-1.5">
        <p className={`text-[11px] font-extrabold ${groupOn ? 'text-[#c9a8ff]' : 'text-white/70'}`}>{g.name}</p>
        <span className={`rounded-md px-1.5 text-[10px] font-bold ${groupOn ? 'bg-jadeed-purple/40 text-[#c9a8ff]' : 'bg-white/10 text-white/40'}`}>{items.length}</span>
      </div>
        ...
        {roots.map((s) => {
          ...
          return (
            <div key={s.key}>
              ...
              >
                <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${on ? 'bg-jadeed-orange' : 'bg-white/15 group-hover:bg-white/30'}`} />
                <span className={`shrink-0 font-mono text-[10px] ${on ? 'text-[#c9a8ff]' : 'text-white/35'}`} dir="ltr">{s.id}</span>
                <span className={`truncate text-xs ${on ? 'font-extrabold text-white' : 'font-bold text-white/70'}`}>{s.title}</span>
                {s.badge && <span className="shrink-0 rounded-md bg-jadeed-orange/90 px-1.5 py-px text-[9px] font-extrabold text-white">{s.badge}</span>}
                {s.star && <span className="shrink-0 text-[10px] text-jadeed-yellow" title="الشاشة المعيارية">★</span>}
                {subs.length > 0 && (
                  <span className="ms-auto shrink-0 rounded-md bg-white/10 px-1.5 text-[9px] font-bold text-white/40">+{subs.length} حالات</span>
                )}
              </button>
              {subs.length > 0 && (
                <div className="ms-5 space-y-0.5 border-s-2 border-white/10 py-0.5 ps-2.5">
                  {subs.map((sub) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`font-mono text-[9px] ${onS ? 'text-[#c9a8ff]' : 'text-white/30'}`} dir="ltr">{sub.id}</span>
                        <span className={`truncate text-[11px] ${onS ? 'font-extrabold text-white' : 'font-bold text-white/50 group-hover:text-white/75'}`}>{sub.title}</span>
                        {sub.badge && <span className="shrink-0 rounded-md bg-jadeed-orange/80 px-1 py-px text-[8px] font-extrabold text-white">{sub.badge}</span>}
                      </button>
                    )
                  })}
                </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const sync = () => { setIsRow(row.matches); setMd(mid.matches); setXl(wide.matches) }
    sync()
  ...
  const filtered = useMemo(() => {
    ...
    if (!t) return null
    return curApp.screens.filter((s) => `${s.id} ${s.title}`.toLowerCase().includes(t))
  }, [q, curApp])

  const step = (d) => setKey(ALL_SCREENS[(idx + d + ALL_SCREENS.length) % ALL_SCREENS.length].key)
      ...
      <AStoreProvider>
        <MStoreProvider>
          <div className="flex h-screen flex-col overflow-hidden bg-jadeed-bg font-tajawal text-jadeed-black sm:flex-row">
            ...
            {/* ═══ القائمة الجانبية — قابلة للطي بحركة زنبركية ═══ */}
            <motion.aside
              initial={false}
              animate={isRow ? { width: collapsed ? 0 : SIDEBAR_W } : { width: '100%' }}
              ...
              >
                <div className="flex items-center gap-3 px-5 pb-2 pt-4">
                  ...
                  <div className="min-w-0 grow">
                    <p className="text-sm font-extrabold leading-5">جديد · النموذج التفاعلي</p>
                    <p className="text-[10px] text-white/45">عميل + تاجر + إدارة — Workbench شامل</p>
                  </div>
                  <span className="rounded-lg bg-jadeed-orange px-2 py-1 text-[10px] font-extrabold">V3</span>
                  {/* زر الطي — يشير لليمين (اتجاه انقباض السايدبار في RTL) */}
                  <button
                ...
                <div className="grid grid-cols-3 gap-1.5 px-4 pb-2 pt-1">
                  {APPS.map((a) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`flex h-8 w-8 items-center justify-center rounded-xl text-white ${on ? a.accent : 'bg-white/10'}`}>
                          <a.Icon size={15} />
                        </span>
                        <span className={`text-[10px] ${on ? 'font-extrabold text-white' : 'font-bold text-white/50'}`}>{a.short}</span>
                        <span className="font-mono text-[8px] text-white/30" dir="ltr">{a.hint}</span>
                      </button>
                    )
                  })}
                </div>

                {/* بحث داخل التطبيق النشط */}
                <div className="px-4 pb-1.5">
                  <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 focus-within:border-jadeed-purple-light">
                    ...
                    <span className="shrink-0 rounded-md bg-white/10 px-1.5 py-0.5 text-[10px] font-bold text-white/50">
                      {filtered ? filtered.length : curApp.screens.length}/{curApp.screens.length}
                    </span>
                  </div>
                </div>

                {/* شجرة التطبيق النشط فقط */}
                <div className="side-scroll min-h-0 grow overflow-y-auto px-3 pb-2">
                  {curApp.groups.map((g) => {
                    const items = filtered ? filtered.filter((s) => s.group === g.id) : curApp.screens.filter((s) => s.group === g.id)
                    if (!items.length) return null
                    return <GroupTree key={g.id} g={g} items={items} curKey={key} go={go} />
                  })}
                  {filtered && filtered.length === 0 && (
                    <p className="px-3 py-6 text-center text-xs text-white/40">لا نتائج في هذه اللوحة</p>
                  )}
                </div>
            ...
            >
              <header className="z-20 flex h-11 shrink-0 items-center justify-between gap-3 border-b border-jadeed-line bg-white/80 px-4 backdrop-blur">
                <div className="flex min-w-0 items-center gap-2.5">
                  {/* زر فتح القائمة — يظهر فقط وهي مطوية */}
                  <AnimatePresence>
                    {collapsed && isRow && (
                      <motion.button
                        key="expand-sidebar"
                  ...
                  <p className="flex min-w-0 items-center gap-1.5 truncate text-[13px] font-extrabold">
                    ...
                    {cur.badge && <span className="shrink-0 rounded-md bg-jadeed-orange px-1.5 py-px text-[9px] font-extrabold text-white">{cur.badge}</span>}
                    {cur.star && <span className="shrink-0 rounded-md bg-jadeed-tint px-1.5 py-px text-[9px] font-extrabold text-jadeed-purple">★ المعيار</span>}
                    {curApp.desktop && <span className="hidden shrink-0 rounded-md bg-jadeed-gray px-1.5 py-px text-[9px] font-extrabold text-jadeed-muted sm:block">عرض سطح مكتب</span>}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  <span className="mx-1 hidden text-[11px] font-bold text-jadeed-muted sm:block" dir="ltr">
                    {idx + 1} / {ALL_SCREENS.length}
                  </span>
                  <button onClick={() => step(1)} title="السابق (→)" className="rounded-lg border border-jadeed-line bg-white p-1.5 shadow-soft transition hover:text-jadeed-purple">
                    <ChevronRight size={15} />
                  </button>
                  <button onClick={() => step(-1)} title="التالي (←)" className="rounded-lg border border-jadeed-line bg-white p-1.5 shadow-soft transition hover:text-jadeed-purple">
                    <ChevronLeft size={15} />
                  </button>
                </div>
              </header>
